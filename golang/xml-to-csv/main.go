...
func main() {
	// open xml file
	xmlFile, err := os.Open("data/customers.xml")
	if err != nil {
		log.Fatal(err)
	}
	...
	// convert xml file to bytes
	xmlBytes, _ := io.ReadAll(xmlFile)

	// create customers.csv
	customerCSV, err := os.Create("data/customers.csv")
	if err != nil {
		log.Fatal(err)
	}

	// create orders.csv
	orderCSV, err := os.Create("data/orders.csv")
	if err != nil {
		log.Fatal(err)
	}
	...
	xml.Unmarshal(xmlBytes, &purchaseData)
